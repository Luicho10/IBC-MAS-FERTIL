...
function EditableRow({ data, index, fields, collection, setData }) { return <tr>{fields.map(field => <td key={field}><input value={data[field] || ''} onChange={e => setData(collection, index, field, e.target.value)} /></td>)}</tr>; }
function LogoHeader() { return <header className="document-header"><img src="/logo-mas-fertil.svg" alt="másfertil fertilizantes" className="mas-logo" /><div className="granu"><span>GRANU</span><b>+</b></div></header>; }
function PageFooter() { return <footer>FORM. IBCTIM. V6 MFSSETIEMBRE 26</footer>; }
...
export default function App() {
  ...
  return <>
    <div className="toolbar no-print"><div><b>IBC MAS FERTIL</b><small>Relevamiento Técnico Integral y Multisectorial — V6</small></div><div className="toolbar-actions"><button className="secondary" onClick={() => setD(nuevoEstado())}>NUEVO IBC</button><button onClick={() => window.print()}>GENERAR PDF / IMPRIMIR</button></div></div>
    <main className="document">
      <article className="print-page page-one">
        <LogoHeader /><h1>(IBC) TÉCNICO INTEGRAL Y MULTISECTORIAL</h1><Field label="FECHA" value={d.fecha} onChange={v => set('fecha', v)} type="date" className="date-field" />
      ...
      <article className="print-page page-two">
      ...
      <article className="print-page page-three">
