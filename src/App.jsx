    ...
    <tr>
      {fields.map(field => (
        <td key={field}>
          <input
            value={data[field] || ''}
            onChange={e => setData(collection, index, field, e.target.value)}
          />
        </td>
      ))}
    </tr>
...
export default function App() {
  ...
  const setRow = (collection, index, field, value) => {
    setD(prev => ({
      ...prev,
      [collection]: prev[collection].map((row, i) => i === index ? { ...row, [field]: value } : row)
    }));
  };
  ...
  return (
    <>
      <div className="toolbar no-print">
        <div>
          <b>IBC MAS FERTIL</b>
          <small>Relevamiento Técnico Integral y Multisectorial</small>
        </div>
        <div className="toolbar-actions">
          <button className="secondary" onClick={() => setD(nuevoEstado())}>NUEVO IBC</button>
      ...
      <main className="document">
        ...
        <article className="print-page">
          <header className="document-header">
            <div className="logo">más<span>fertil</span><small>FERTILIZANTES</small></div>
            <div className="granu">GRANU+</div>
          </header>
          ...
          <Section number="1" title="DATOS DEL CLIENTE">
            <div className="checks client-type">
              <label className="check"><input type="radio" checked={d.tipo === 'Persona Física'} onChange={() => set('tipo', 'Persona Física')} /> Persona Física</label>
              <label className="check"><input type="radio" checked={d.tipo === 'Persona Jurídica'} onChange={() => set('tipo', 'Persona Jurídica')} /> Persona Jurídica</label>
            </div>
            <div className="grid two">
              <Field label="NOMBRE / RAZÓN SOCIAL" value={d.nombre} onChange={v => set('nombre', v)} />
              <Field label="C.I. / RUC" value={d.ci} onChange={v => set('ci', v)} />
              <Field label="TELÉFONO" value={d.telefono} onChange={v => set('telefono', v)} />
              <Field label="DOMICILIO / CIUDAD" value={d.domicilio} onChange={v => set('domicilio', v)} />
              <Field label="E-MAIL" value={d.email} onChange={v => set('email', v)} />
            </div>
          </Section>
          ...
          <Section number="2" title="PERFIL DE ACTIVIDAD Y SUPERFICIE">
            ...
            <div className="no-print table-actions">
              <button className="link" onClick={() => addRow('fincas', finca)}>+ Agregar ubicación</button>
              {d.fincas.length > 1 && <button className="link danger" onClick={() => removeRow('fincas', d.fincas.length - 1)}>- Quitar última</button>}
            </div>
          </Section>

          <Section number="2.1" title="MÓDULO COMERCIAL Y DE SERVICIOS">
            <div className="label-line">REVENTA:</div>
            <Checks items={['Agroquímicos', 'Fertilizantes', 'Semillas']} value={d.reventa} onChange={v => set('reventa', v)} />
            <div className="label-line">SERVICIOS:</div>
            <Checks items={['Consultoría', 'Maquinaria Pesada', 'Logística', 'Asistencia Técnica', 'Acopio Silo']} value={d.servicios} onChange={v => set('servicios', v)} />
          </Section>
          ...
          <Section number="3" title="PLAN DE PRODUCCIÓN">
            <Table heads={['CULTIVO', 'Has. ANT.', 'Rnd. Kg', 'Has. Actual', 'TN Estimada']} className="production-table">
              {d.prod.map((row, i) => (
                <tr key={row.cultivo}>
                  <td><b>{row.cultivo}</b></td>
                  {['ant', 'rnd', 'actual', 'tn'].map(field => <td key={field}><input value={row[field]} onChange={e => setRow('prod', i, field, e.target.value)} /></td>)}
                </tr>
              ))}
            </Table>
          </Section>
        ...
        {/* PÁGINA 2 */}
        <article className="print-page">
          <header className="document-header compact-header">
            <div className="logo">más<span>fertil</span><small>FERTILIZANTES</small></div>
            <div className="granu">GRANU+</div>
          </header>

          <Section number="3.1" title="FECHAS PRODUCTIVAS">
            <div className="grid three">
              <Field label="Fecha estimada de siembra" value={d.siembra} onChange={v => set('siembra', v)} type="date" />
          ...
          <Section number="4" title="MANIFESTACIÓN DE BIENES Y ACTIVOS PRODUCTIVOS">
            ...
            <Table heads={['Tipo / Equipo / Vehículo', 'Marca / Modelo', 'Año', 'Valor mercado Gs/Usd', 'Deuda / Prenda (SI/NO)']}>
              {d.bienes.map((row, i) => <EditableRow key={i} data={row} index={i} collection="bienes" fields={['tipo', 'marca', 'anio', 'valor', 'deuda']} setData={setRow} />)}
            </Table>
            <div className="no-print table-actions">
              <button className="link" onClick={() => addRow('bienes', bien)}>+ Agregar bien</button>
              {d.bienes.length > 1 && <button className="link danger" onClick={() => removeRow('bienes', d.bienes.length - 1)}>- Quitar última</button>}
            </div>

            <h3>4.2 GANADO / SEMOVIENTES</h3>
            <Table heads={['Especie / Categoría', 'Cantidad', 'Valor unitario Gs/Usd', 'Valor total Gs/Usd', 'Gravamen (SI/NO)']}>
              {d.ganado.map((row, i) => <EditableRow key={i} data={row} index={i} collection="ganado" fields={['especie', 'cantidad', 'unitario', 'total', 'gravamen']} setData={setRow} />)}
            </Table>
            <div className="no-print table-actions">
              <button className="link" onClick={() => addRow('ganado', ganado)}>+ Agregar ganado</button>
              {d.ganado.length > 1 && <button className="link danger" onClick={() => removeRow('ganado', d.ganado.length - 1)}>- Quitar última</button>}
            </div>
          </Section>
          <PageFooter />
        </article>
        ...
        <article className="print-page">
          ...
          <Section number="5" title="REFERENCIAS">
            <Table heads={['ENTIDAD', 'CONTACTO', 'TELÉFONO']}>
              {d.refs.map((row, i) => (
                <tr key={row.tipo}>
                  <td><b>{row.tipo}:</b></td>
                  <td><input value={row.contacto} onChange={e => setRow('refs', i, 'contacto', e.target.value)} /></td>
                  <td><input value={row.telefono} onChange={e => setRow('refs', i, 'telefono', e.target.value)} /></td>
                </tr>
              ))}
            </Table>
          </Section>
          ...
          <Section number="6" title="DECLARACIÓN Y AUTORIZACIÓN">
            ...
            <div className="grid two declaration-fields">
              <Field label="TITULAR/REPRESENTANTE LEGAL" value={d.titular} onChange={v => set('titular', v)} />
              <Field label="RUC/CI N°" value={d.ruc} onChange={v => set('ruc', v)} />
            </div>

            <div className="signature">
              <div className="signature-line" />
              <b>TITULAR/REPRESENTANTE LEGAL</b>
              <Field label="ACLARACION" value={d.aclaracion} onChange={v => set('aclaracion', v)} />
              <Field label="RUC/CI N°" value={d.ruc} onChange={v => set('ruc', v)} />
            </div>
          </Section>
          <PageFooter />
